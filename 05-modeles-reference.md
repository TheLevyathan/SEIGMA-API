...
| Problème | Impact | Workaround |
|----------|--------|------------|
| **SalesOrderId TRANSITIF** | → voir ch.7 [pitfall #8](07-pitfalls-limitations.md#8-salesorderid-est-transitif-sur-receipt). La relation est `Receipt → SalesInvoice → SalesOrder`. |
...
| Problème | Impact | Workaround |
|----------|--------|------------|
| **WhereCondition par `SalesOrderId` → 500** | Toute recherche filtrant sur `SalesOrderId` retourne une erreur SQL Server : `"The multi-part identifier dbo.CallView.SalesOrder.Display_2 could not be bound."` | Récupérer tous les Calls sans filtre, puis filtrer côté client sur `SalesOrderId.ReferenceId` |
...
**⚠️ Quirks :**
- `GET /api/reference/Project/{id}` retourne **500 Internal Server Error** (bug SEIGMA — le serveur retourne une erreur 500 au lieu de 404).
